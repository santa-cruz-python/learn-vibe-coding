...
Coding harnesses have slash commands — typed as `/init` — that set a project up, which help create an AGENTS.md or CLAUDE.md file (as mentioned in [Model, Agent, Harness and Loops](../../concepts/model-agent-harness-loops/#agentsmd-and-claudemd-files), prefer AGENTS.md for compatibility, so if Claude Code writes a CLAUDE.md file, just rename it AGENTS.md).
...
If you don't have an engineering background, before you even start on your project, you may want to start by asking what good software development practices you should write into the AGENTS.md file, and learn a bit about how software is typically developed.
...
Credentials — passwords, API keys and anything else that proves who you are — are usually supplied as environment variables: named values your program reads from its surroundings instead of from its own code. They are normally kept in a file called `.env`. If you have multiple environments, you may have multiple `.env` files. A critical security consideration is to make sure credentials are never shared, so ensure the file is ignored by git and prevented from being committed by adding it to the `.gitignore` file. Ask your agent to ensure this. This is typically enforced by agents, but also worth specifying in the AGENTS.md file.
