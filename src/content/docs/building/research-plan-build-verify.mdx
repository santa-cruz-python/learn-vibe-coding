...
Do you have an idea in mind, and you know what you want, or is there still grey areas? Either way, it's usually a great idea to start with research.
...
- what:
    what does the product do? research what each user persona wants to achieve with this product. This is the time to be creative and find all the possible usage and features a user may want or need.

    What are the minimal features needed to launch the product? what persona(s) should be targeted first? what will the Minimal Viable Product (MVP) include (must do, could do, won't do)
...
:::tip[Pro Tip]
As a general guidance: think big, start small. Starting with a small idea and growing it organically tends to produce very messy code bases and performance issues in the long run. Think big, think of all the things the product may do in the future, and document this future state, so whenever you work on a feature, the AI agent understands where it fits in the big picture; it is much more likely to chose the proper technology stack and architecture if it knows the end-goal. Starting small, the choices are endless. It is easy for AI to pick a technological solution that seems simple and cheap at the time, but won't scale or grow with the product as it needs to.
:::
...
AI tends to recommend 'most popular' solutions, and might box you into using services or platforms that are meant for testing or proof of concepts (PoC), like using SQLite for a database. SQLite is not a toy, and plenty of real products run on it: reads are unlimited and concurrent, and read-heavy applications scale a very long way on it. But it allows exactly one writer at a time, it lives in a single file so you inherit whatever size ceiling the filesystem imposes, and it is not safe over a network filesystem. If your product needs concurrent writes, or several servers talking to the same database, that is the wall you will hit, and it is much better to know it before you build on top of it than after. Make sure you understand the choices made and their downsides.
:::
