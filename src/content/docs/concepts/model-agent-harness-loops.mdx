...
An Agent is a unit that makes calls to LLMs. An agent usually has instructions (System prompt, user prompt) and may have access to tools. Tool definitions are part of the prompt: the LLM generally does not run the tools; it decides if it needs to run a tool to perform a task, and provides a structured output to tell the client what tool to call with what parameters. The agent calling the LLM then can run the tool, and provide the output of the tool to the next turn of the conversation for the LLM to evaluate.
