...
To register a Codex-compatible marketplace:

```sh
codex plugin marketplace add owner/repo
```

(it also accepts a `--ref <branch>`, a Git URL, or a local path; manage what's already registered with `codex plugin marketplace list`, `upgrade`, and `remove`.)

Installing, though, isn't a single copy-paste command the way it is in Claude Code — Codex doesn't ship a scriptable `codex plugin install`. Open a Codex session and run `/plugins` to browse the plugins available from your configured marketplaces and install one from there. Verified against OpenAI's own [Codex plugins](https://developers.openai.com/codex/plugins) and [building Codex plugins](https://developers.openai.com/codex/plugins/build) docs.
